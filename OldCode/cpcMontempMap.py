...
if(mm != '00'):
	for record, shape in zip(records, shapes):
		...
		wgs84 = pyproj.Proj(init='epsg:5070')
...
if(mm != '00'):
	for record, shape in zip(records, shapes):
		...
		if(record[3] == 'Above'):
			if(dval >= 70): col = '#ad151c'
			if(dval == 60): col = '#c72c34'
			if(dval == 50): col = '#d65440'
			if(dval == 40): col = '#d98359'
			if(dval == 33): col = '#dba670'
			lines.set_facecolors(col)
			lines.set_edgecolors(col)
			lines.set_linewidth(1.0)
			lines.set_zorder(10)
		...
		if(record[3] == 'Below'):
			if(dval >= 70): col = '#0871a6'
			if(dval == 60): col = '#36a1d6'
			if(dval == 50): col = '#76b8de'
			if(dval == 40): col = '#a0bfd9'
			if(dval == 33): col = '#c1d1e3'
			lines.set_facecolors(col)
			lines.set_edgecolors(col)
			lines.set_linewidth(1.0)
			lines.set_zorder(10)
...
if(imgsize == 'GEO'):
	...
	for nshape,seg in enumerate(m.states):
		if statenames[nshape] in ['Hawaii']:
			poly = Polygon(seg, facecolor='#e3e3e3',edgecolor='#525252', linewidth=0.3, zorder=10)
			ax1.add_patch(poly)
...
if(imgsize == 'GEO'):
	plt.savefig(outpng, dpi=figdpi, orientation='landscape', transparent='true', bbox_inches='tight', pad_inches=0.00)
